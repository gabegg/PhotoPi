"""
    Graphical and Command-line setup scripts
"""
import os.path
import sys
...
class Assistant(Tk):
    ...
    def __init__(self,config,*args,**kwargs):
        ...
        try:
            ...
            self.want_email_cb  = Checkbutton(self.main_frame, text="Enable option to send to email", variable=self.want_email_var, anchor=W, font='Helvetica')
            self.want_upload_cb  = Checkbutton(self.main_frame, text="Enable option to upload to Google Photos", variable=self.want_upload_var, anchor=W, font='Helvetica')
            ...
            if printer_selection_enable == True:
                self.want_print_cb = Checkbutton(self.main_frame, text="Enable option to print photo", variable=self.want_print_var, anchor=W, font='Helvetica')
            ...
            self.use_soft_keyboard_cb = Checkbutton(self.main_frame, text="Enable onscreen keyboard", variable=self.use_soft_keyboard_var, anchor=W, font='Helvetica')
            if self.printer_selection_enable == True:
                ...
                def item_selected(*args):
                    ...
                    if displayed_list_names[cursel] == "<Create New>":
                        try:
                            ...
                            album_id = self.google_service.create_album(album_name = "Photo Pi Test", add_placeholder_picture = True)
                            self.album_id_var.set(album_id)   
                            self.album_name_var.set("Photo Pi Test")
...
def console_assistant():
    ...
    We're all set, I just created a script to launch Photo Pi with
